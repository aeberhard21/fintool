    ...
    for txn in transactions { 
        ...
        if txn.classification.is_some() {
            match txn.classification.clone().unwrap().as_str() {
                ...
                _ => {
                    eprintln!("Unrecognized classification type: {}", txn.classification.unwrap());
                    std::process::exit(1);
                }
            };
        } else { 
        ...
        // println!("Transaction:\n\t{:?}", ledger_entry);
        println!("{}, {}, {}, {}, {}, {}", ledger_entry.date, ledger_entry.amount, ledger_entry.transfer_type as u32, ledger_entry.participant, ledger_entry.category_id, ledger_entry.description);
    }
}