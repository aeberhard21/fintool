...
use super::{DbConn, db_investments::StockRecord, db_banks::BankRecord, SQLITE_WILDCARD};
...
impl DbConn {
    ...
    pub fn record_hsa_account(&mut self, aid: u32, record: HsaRecord) -> Result<(), Error> {
        ...
        self.conn.execute(sql, rusqlite::params!(record.fixed.date, record.fixed.amount, aid)).expect(fmsg.as_str());
    ...
    pub fn get_hsa_value(&mut self, aid: u32) -> Result<HsaRecord, rusqlite::Error> {
        let bank = self.get_bank_value(aid).expect("Unable to retrieve HSA account record!");
